// BC ProffieOS 5.9 just_booted_ version w/ idle.bmp and Aurebesh option
          ...
          if (looped_frames_ == 1) {
            ...
            if (looped_frames_ == frame_count_) {
              if (millis() - loop_start_ > effect_display_duration_) {
                STDOUT << "----------------- if (millis() - loop_start_ > effect_display_duration\n";
                if (!SaberBase::Lockup()) {
                  screen_ = SCREEN_PLI;
                  if (looped_on_) {
                    STDOUT << "----------------- (looped_on_)       ShowFile(&IMG_on,\n";
