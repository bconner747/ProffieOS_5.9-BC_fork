#ifndef SOUND_HYBRID_FONT_H   // BC Contains AUTO_HUMSTART, SPINS BLAST, and LOW_BATT_ONCE mods.
...
public:
  void SetVariable(const char* variable, float v) override {
    CONFIG_VARIABLE(humStart, 1000);                            // BC was only 100
...
public:
...
#ifdef ENABLE_AUTO_SPINS_BLAST                // BC this is not where I want to do this. Should be in prop file, but how? It gets detected here...?
  ...
  #ifdef AUTO_HUMSTART                      // BC Auto Humstart at 200ms
      hum_start_ += 200;
      STDOUT << "Auto-humstart: 200ms \n";
  #else
  ...
  void SB_Effect(EffectType effect, float location) override {
	...
	// If no stab sounds are found, fall through to clash
      ...
      case EFFECT_BLAST: Play(&SFX_blaster, &SFX_blst); last_blast_ = millis(); return;         // BC this is not where I want to do this, should be prop...but how?
...
#ifdef LOW_BATT_ONCE           // BC add for limiting Low Batt notifiation to once
 ...
 private:
  ...
  uint32_t hum_start_;
  uint32_t last_blast_ = millis();  // BC add  
  float hum_fade_in_;
  float hum_fade_out_;
#ifdef ENABLE_SPINS
  float angle_;
#endif
  bool low_batt_notify_ = false; // BC this is not where I want to do this, should be prop...but how?
