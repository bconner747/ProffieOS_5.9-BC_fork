...
public:
  ...
  void PlayOnce(Effect* effect, float start = 0.0) {
    ...
    if (effect->Play(filename_)) {
      effectname_ = effect->name_; // set effect name attribute to wav_players  // BC obirob WavLenMod  
      start_ = start;
      effect_ = nullptr;
      run_ = true;
    }
  ...
  void PlayLoop(Effect* effect) {
    effect_ = effect;
    if(effect_->name_[0] > 96 && effect_->name_[0] < 123){ // is it readable ascii??  // /* BC obirob WavLenMod
...
public:
  ...
  const char* effectname() const {          // /* BC obirob WavLenMod
    return effectname_;
  }
...
private:
  ...
  char filename_[128];
  const char* effectname_;                  // BC obirob WavLenMod
  const char* effect2ndname_;               // BC obirob WavLenMod
