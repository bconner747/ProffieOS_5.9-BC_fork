...
#define CONFIG_FILE "config/BC_sample_config.h"
...
#include "functions/wav_time.h" // BC for variable in/out
// transitions
  ...
  public:
    ...
    void Loop() override {
      ...
      while (true) {
        ...
        if (!SA::AlwaysConnected()) {
          STDOUT.println("Welcome to ProffieOS 5.9-BC, type 'help' for more info.");
        }
    ...
    void ParseLine() {
      ...
      if (!CommandParser::DoParse(cmd, e)) {
        STDOUT.print("What? Typo? or is Disable Diagnostic Commands enabled? :");        //BC custom phrase
        STDOUT.println(cmd);
      }
