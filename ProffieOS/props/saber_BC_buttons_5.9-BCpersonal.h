/*      // saber_BC_buttons_5.9v3_Personal Version with:
...
Best if used with included custom wav files in a 'common' font folder.
...
// Hum always start after 200ms of ignition.                     // BC personal only
// Auto Spins blaster blocks.                                    // BC personal only 
// Low battery warning only once,                                // BC personal only
// Use wav files for talkie things.                              // BC personal only
...
EFFECT_USER3            - for Rave glowstick, instead of preon.  // BC personal Version only
...
// BC PERSONAL Version ONLY UNTIL I FIGURE OUT HOW TO DO IN PROP ONLY ///                   // BC Personal Version Only
// #define ENABLE_AUTO_SPINS_BLAST - Same as Auto Swing Blast mode.....but for Spins,
//                                   with a 2 second window.
//                                   **Requires modified hybrid_font.h file.
//                                   and #define ENABLE_SPINS                                  
...
// #define USE_WAVS_FOR_TALKIE      - This will use wav files instead of built in Talkie.
                                     **Requires custom numerals and battery level wav files.

#define PREONS_MATCH_OUTS          - Matching preon/out and pstoff/in wavs.
#define PSTOFFS_MATCH_INS          - These defines will have the OS play preon01 with out01 etc...
                                     and pstoff01 with in01 etc...
                                     If defined, AND if there are the same number of both files they will get matched. 
                                     If an odd number, they will play randomly as usual.
                                     **Requires modified prop_base.h
...
EFFECT_USER3          - Clash while off and EFFECT_USER3 is in blade style.       // BC Personal Version Only
...
EFFECT(battlevl);  // for Spoken Battery Level
EFFECT(point);      // for Spoken Battery Level
EFFECT(volts);      // for Spoken Battery Level
EFFECT(percent);    // for Spoken Battery Level
EFFECT(rave);       // for Glow Stick Preon
...
// // Match preon.wav to out.wav -------------------> Matching preon/out and pstoff/in moved to prop_base for universiality
...
// Gesture Ignition Controls
  ...
  //#ifndef PSTOFFS_MATCH_INS
        //SelectRetractionPair();-------------------> Matching preon/out and pstoff/in moved to prop_base for universiality
  //#endif
        Off();
        last_twist_ = millis();
...
// Spoken Battery Level - BC use custom talkie.h
// Spoken Battery Level in volts
    case EVENTID(BUTTON_POWER, EVENT_THIRD_SAVED_CLICK_SHORT, MODE_OFF):
      if (!mode_volume_) {
  #ifdef USE_WAVS_FOR_TALKIE            // BC Personal only because delay() is not great
        hybrid_font.PlayCommon(&SFX_battlevl);
        delay (1900);
        talkie.SayDigitWav((int)floorf(battery_monitor.battery()));
        delay (600);
        hybrid_font.PlayCommon(&SFX_point);
        delay (600);
        talkie.SayDigitWav(((int)floorf(battery_monitor.battery() * 10)) % 10);
        delay (600);
        talkie.SayDigitWav(((int)floorf(battery_monitor.battery() * 100)) % 10);
        delay (700);
...
// Spoken Battery Level in percentage
    case EVENTID(BUTTON_POWER, EVENT_THIRD_HELD, MODE_OFF):
      if (!mode_volume_) {
        ...
        hybrid_font.PlayCommon(&SFX_battlevl);
        delay (1900);
        talkie.SayNumberWav((int)floorf(battery_monitor.battery_percent()));
        delay (670);
        hybrid_font.PlayCommon(&SFX_percent);
...
// Turns Saber ON
    ...
    //#ifdef PREONS_MATCH_OUTS
        //SelectIgnitionPair();-------------------> Matching preon/out and pstoff/in moved to prop_base for universiality
    //#endif
      // Bypass preon if pointing up         
        if (fusor.angle1() >  M_PI / 3) {
          FastOn(); // Not needing the sound though since out.wav still plays simultaneously...unless wanted faston.wav also.
        } else {
          On();
        }
      }
...
// USER3 Effect
    case EVENTID(BUTTON_NONE, EVENT_CLASH, MODE_OFF):
      hybrid_font.DoEffect(EFFECT_USER3, 0);
      return true;
  ...
  void SB_Effect(EffectType effect, float location) override {
    switch (effect) {
      ...
      case EFFECT_USER2: // Monoforce
        ...
        return;
      case EFFECT_USER3: // Rave
        if (SFX_rave) hybrid_font.PlayCommon(&SFX_rave);
        return;
    }
  }
