// BC 5.9v3 Includes PREONS_MATCH_OUTS / PSTOFFS_MATCH_INS
...
// Match preon.wav to out.wav //--------------------- BC not official yet
...
// #ifdef ANGLE_PREON                                      // BC preon that only works if the blade is angled down
...
//     }
// #else
#ifdef PREONS_MATCH_OUTS     //--------------------- BC not official yet
      SelectIgnitionPair();  //--------------------- BC not official yet
#endif                       //--------------------- BC not official yet
       SaberBase::DoPreOn(&preon_time);
      if (preon_time > 0.0) {
        on_pending_ = true;
        on_pending_base_ = millis();
...
// Match pstoff.wav to in.wav //--------------------- BC not official yet
...
#endif    
#ifdef PSTOFFS_MATCH_INS    //--------------------- BC not official yet
    SelectRetractionPair(); //--------------------- BC not official yet
#endif                      //--------------------- BC not official yet
...
#define DEACTIVATE(N) do {                      \
  ...
  void SB_Message(const char* text) override {
    STDOUT.print("DISPLAY: \n");                // BC add return for clean 2 line
    STDOUT.println(text);
  }
...
#define CHECK_SUPPORTS_COLOR_CHANGE(N) \
  ...
  STDOUT.println("OFF");
    ...
    if (!strcmp(cmd, "beep")) {
      // Close Encounters                   // BC added songs :)
